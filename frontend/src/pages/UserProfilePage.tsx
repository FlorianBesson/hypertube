...
export default function UserProfilePage({
  ...
  useEffect(() => {
    ...
    fetchProfile()
  }, [targetUserId, t.errorLoading])

  const formattedCreatedAt = profile?.createdAt
    ? new Date(profile.createdAt).toLocaleDateString(lang === 'fr' ? 'fr-FR' : 'en-US', {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
      })
  ...
  const formattedLastLogin = profile?.lastLogin
    ? new Date(profile.lastLogin).toLocaleDateString(lang === 'fr' ? 'fr-FR' : 'en-US', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
        hour: '2-digit',
        ...
        {loading ? (
          ...
          <div className="w-full flex flex-col gap-6">
            ...
            <div className="bg-neutral-900/60 border border-red-500/20 rounded-2xl p-10 backdrop-blur-md text-center flex flex-col gap-4">
              ...
              <Button onClick={() => navigate('/dashboard')} className="max-w-xs mx-auto mt-2">
                {t.backToDashboard}
              </Button>
            </div>
          </div>
          ...
          <div className="w-full flex flex-col gap-6">
            ...
            <div className="flex items-center gap-4 bg-neutral-900/40 border border-white/5 rounded-2xl p-4 backdrop-blur-md">
              ...
              <div>
                <h1 className="text-xl font-bold tracking-tight text-white">{t.userProfile}</h1>
                <p className="text-xs text-neutral-400 mt-0.5">{lang === 'fr' ? 'Consulter le profil public de ce membre' : 'Viewing public profile details of this member'}</p>
              </div>
            </div>

            {/* Profile Content Layout (Grid) */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
              ...
              <div className="md:col-span-2 bg-neutral-900/50 border border-white/5 rounded-2xl p-6 sm:p-8 flex flex-col gap-6 backdrop-blur-md hover:border-white/10 transition-all duration-300">
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex items-center gap-2 border-b border-white/5 pb-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 text-red-500">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5zm6-10.125a1.875 1.875 0 11-3.75 0 1.875 1.875 0 013.75 0zm-1.2 6.477a3 3 0 00-5.1 0 1.875 1.875 0 001.35 2.648h2.4a1.875 1.875 0 001.35-2.648z" />
                    </svg>
