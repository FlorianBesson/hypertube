...
/** Polls the P2P seed count, video container format and HLS conversion status for the current stream while no stream error is active. */
export function useStreamStats(streamIdentifier: string, disabled: boolean, token: string | null): StreamStats {
  const [stats, setStats] = useState<StreamStats>({ seeds: null, format: null, conversionStatus: null })
