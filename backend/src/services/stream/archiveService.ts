...
function streamLocalFile(filePath: string, rangeHeader: string | undefined, res: any, filename: string): void {
...
async function backgroundDownloadArchiveMovie(identifier: string, directUrl: string, targetPath: string, fileSize: bigint | null, userAgent?: string): Promise<void> {
...
export async function streamArchiveMovie(
  ...
  let data: any = null;
  try {
    const metaRes = await fetchWithTimeout(`https://archive.org/metadata/${encodeURIComponent(identifier)}`, {
